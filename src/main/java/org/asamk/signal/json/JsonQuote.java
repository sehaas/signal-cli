...
public class JsonQuote {
	...
	JsonQuote(SignalServiceDataMessage.Quote quote, Manager m) {
		...
		if (quote.getMentions() != null && quote.getMentions().size() > 0) {
			this.mentions = quote.getMentions().stream().map(quotedMention -> new JsonMention(quotedMention, m))
					.collect(Collectors.toList());
		}

		if (quote.getAttachments().size() > 0) {
			this.attachments = quote.getAttachments().stream().map(JsonQuotedAttachment::new)
					.collect(Collectors.toList());
		} else {
