...
public class JsonDbusReceiveMessageHandler extends JsonReceiveMessageHandler {
			...
			if (content.getReceiptMessage().isPresent()) {
				...
				if (receiptMessage.isDeliveryReceipt()) {
					for (long timestamp : receiptMessage.getTimestamps()) {
						try {
							conn.sendMessage(
									new Signal.ReceiptReceived(objectPath, timestamp, sender.getLegacyIdentifier()));
						} catch (DBusException e) {
							e.printStackTrace();
						}
					}
				...
				if (!message.isEndSession() && (groupId == null
					...
					try {
						conn.sendMessage(new Signal.MessageReceived(objectPath, message.getTimestamp(),
								sender.getLegacyIdentifier(), groupId != null ? groupId : new byte[0],
				...
				if (sync_message.getSent().isPresent()) {
					...
					if (transcript.getDestination().isPresent()
							|| transcript.getMessage().getGroupContext().isPresent()) {
						SignalServiceDataMessage message = transcript.getMessage();
						byte[] groupId = getGroupId(message);

						try {
							conn.sendMessage(new Signal.SyncMessageReceived(objectPath, transcript.getTimestamp(),
									sender.getLegacyIdentifier(),
									transcript.getDestination().isPresent()
											? transcript.getDestination().get().getLegacyIdentifier()
											: "",
	...
	private static byte[] getGroupId(final SignalServiceDataMessage message) {
		return message.getGroupContext().isPresent()
				? GroupUtils.getGroupId(message.getGroupContext().get()).serialize()
				: null;
	}
