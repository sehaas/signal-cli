...
public class SendReactionCommand implements LocalCommand {
	...
	public void attachToSubparser(final Subparser subparser) {
		...
		subparser.addArgument("-e", "--emoji").required(true)
				.help("Specify the emoji, should be a single unicode grapheme cluster.");
		subparser.addArgument("-a", "--target-author").required(true)
				.help("Specify the number of the author of the message to which to react.");
		subparser.addArgument("-t", "--target-timestamp").required(true).type(long.class)
	...
	public int handleCommand(final Namespace ns, final Manager m) {
		...
		try {
			...
			if (ns.getString("group") != null) {
				...
				results = m.sendMessageReaction(emoji, isRemove, targetAuthor, targetTimestamp,
