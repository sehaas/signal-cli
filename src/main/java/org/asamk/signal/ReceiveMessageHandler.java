...
public class ReceiveMessageHandler implements Manager.ReceiveMessageHandler {
		...
		if (envelope.isReceipt()) {
			...
			if (exception != null) {
				if (exception instanceof org.whispersystems.libsignal.UntrustedIdentityException) {
					...
					System.out.println("Use 'signal-cli -u " + m.getUsername() + " listIdentities -n " + e.getName()
							+ "', verify the key and run 'signal-cli -u " + m.getUsername()
							+ " trust -v \"FINGER_PRINT\" " + e.getName() + "' to mark it as trusted");
					System.out.println("If you don't care about security, use 'signal-cli -u " + m.getUsername()
							+ " trust -a " + e.getName() + "' to trust it without verification");
				} else {
					System.out.println(
			...
			if (content == null) {
				...
				System.out
						.println(
								String.format("Sender: %s (device: %d)",
										(sourceContact == null ? "" : "“" + sourceContact.name + "” ")
												+ content.getSender().getLegacyIdentifier(),
				...
				if (content.getSyncMessage().isPresent()) {
					...
					if (syncMessage.getRead().isPresent()) {
						...
						for (ReadMessage rm : syncMessage.getRead().get()) {
							...
							System.out.println("From: " + (fromContact == null ? "" : "“" + fromContact.name + "” ")
									+ rm.getSender().getLegacyIdentifier() + " Message timestamp: "
						...
						if (sentTranscriptMessage.getDestination().isPresent()) {
							...
							for (SignalServiceAddress dest : sentTranscriptMessage.getRecipients()) {
								...
								toBuilder.append(destContact == null ? "" : "“" + destContact.name + "” ")
										.append(dest.getLegacyIdentifier()).append(" ");
							}
							to = toBuilder.toString();
						} else {
							to = "Unknown";
						}
						System.out.println("To: " + to + " , Message timestamp: "
								+ DateUtils.formatTimestamp(sentTranscriptMessage.getTimestamp()));
						if (sentTranscriptMessage.getExpirationStartTimestamp() > 0) {
							System.out.println("Expiration started at: "
					...
					if (syncMessage.getVerified().isPresent()) {
						...
						System.out.println(
								" - " + verifiedMessage.getDestination() + ": " + verifiedMessage.getVerified());
						String safetyNumber = Util.formatSafetyNumber(m.computeSafetyNumber(
					...
					if (syncMessage.getConfiguration().isPresent()) {
						...
						if (configurationMessage.getUnidentifiedDeliveryIndicators().isPresent()) {
							System.out.println(" - Unidentified Delivery Indicators: "
									+ (configurationMessage.getUnidentifiedDeliveryIndicators().get() ? "enabled"
											: "disabled"));
						}
					}
					...
					if (syncMessage.getMessageRequestResponse().isPresent()) {
						final MessageRequestResponseMessage requestResponseMessage = syncMessage
								.getMessageRequestResponse().get();
						...
						if (requestResponseMessage.getGroupId().isPresent()) {
							System.out.println(
									"  Group id: " + Base64.encodeBytes(requestResponseMessage.getGroupId().get()));
						}
						if (requestResponseMessage.getPerson().isPresent()) {
							System.out.println(
									"  Person: " + requestResponseMessage.getPerson().get().getLegacyIdentifier());
						}
					}
					if (syncMessage.getKeys().isPresent()) {
						...
						if (keysMessage.getStorageService().isPresent()) {
							System.out.println("  With storage key length: "
									+ keysMessage.getStorageService().get().serialize().length);
						} else {
				...
				if (content.getCallMessage().isPresent()) {
					...
					if (callMessage.getIceUpdateMessages().isPresent()) {
						...
						for (IceUpdateMessage iceUpdateMessage : iceUpdateMessages) {
							System.out.println("Ice update message: " + iceUpdateMessage.getId() + ", sdp: "
	...
	private void handleSignalServiceDataMessage(SignalServiceDataMessage message) {
		...
		if (message.getReaction().isPresent()) {
			...
			System.out.println(" - Target author: "
					+ m.resolveSignalServiceAddress(reaction.getTargetAuthor()).getLegacyIdentifier());
	...
	private void printMention(SignalServiceDataMessage.Mention mention, Manager m) {
		System.out.println("- "
				...
				+ ": " + mention.getStart() + " (length: " + mention.getLength() + ")");
	}
	...
	private void printAttachment(SignalServiceAttachment attachment) {
		...
		if (attachment.isPointer()) {
			...
			System.out
					.println("  Filename: " + (pointer.getFileName().isPresent() ? pointer.getFileName().get() : "-"));
			System.out.println(
					"  Size: " + (pointer.getSize().isPresent() ? pointer.getSize().get() + " bytes" : "<unavailable>")
							+ (pointer.getPreview().isPresent()
									? " (Preview is available: " + pointer.getPreview().get().length + " bytes)"
									: ""));
