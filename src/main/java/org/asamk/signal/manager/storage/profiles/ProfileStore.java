...
public class ProfileStore {
	...
	public void updateProfile(SignalServiceAddress serviceAddress, ProfileKey profileKey, long now,
			SignalProfile profile, ProfileKeyCredential profileKeyCredential) {
	...
	public static class ProfileStoreDeserializer extends JsonDeserializer<List<SignalProfileEntry>> {
		...
		@Override
		public List<SignalProfileEntry> deserialize(JsonParser jsonParser,
			...
			if (node.isArray()) {
				for (JsonNode entry : node) {
					...
					if (entry.hasNonNull("profileKeyCredential")) {
						try {
							profileKeyCredential = new ProfileKeyCredential(
									Base64.decode(entry.get("profileKeyCredential").asText()));
	...
	public static class ProfileStoreSerializer extends JsonSerializer<List<SignalProfileEntry>> {
		...
		@Override
		public void serialize(List<SignalProfileEntry> profiles, JsonGenerator json,
