...
public class GroupUtils {
	...
	public static void setGroupContext(final SignalServiceDataMessage.Builder messageBuilder,
		...
		if (groupInfo instanceof GroupInfoV1) {
			...
			SignalServiceGroupV2 group = SignalServiceGroupV2.newBuilder(groupInfoV2.getMasterKey())
					.withRevision(groupInfoV2.getGroup() == null ? 0 : groupInfoV2.getGroup().getRevision()).build();
			messageBuilder.asGroupMessage(group);
		}
	}
	...
	public static GroupIdV2 getGroupIdV2(GroupIdV1 groupIdV1) {
		final GroupSecretParams groupSecretParams = GroupSecretParams
	...
	private static GroupMasterKey deriveV2MigrationMasterKey(GroupIdV1 groupIdV1) {
		try {
			return new GroupMasterKey(
					new HKDFv3().deriveSecrets(groupIdV1.serialize(), "GV2 Migration".getBytes(), GroupMasterKey.SIZE));
